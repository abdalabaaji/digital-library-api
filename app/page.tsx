export default function Home() {
  ...
  const endpoints = [
    {
      ...
      color: 'blue',
      items: [
        { method: 'GET', path: '/api/authors', desc: 'Get all authors', example: `curl ${apiUrl}/api/authors` },
        { method: 'GET', path: '/api/authors/:id', desc: 'Get author by ID', example: `curl ${apiUrl}/api/authors/auth001` },
        { method: 'POST', path: '/api/authors', desc: 'Create new author', example: `curl -X POST ${apiUrl}/api/authors \\
  -H "Content-Type: application/json" \\
  -d '{"id":"auth999","name":"Jane Doe","biography":"Acclaimed author","birthYear":1975}'` },
        { method: 'PUT', path: '/api/authors/:id', desc: 'Update author', example: `curl -X PUT ${apiUrl}/api/authors/auth999 \\
  -H "Content-Type: application/json" \\
  -d '{"name":"Jane Smith","birthYear":1975}'` },
        { method: 'DELETE', path: '/api/authors/:id', desc: 'Delete author', example: `curl -X DELETE ${apiUrl}/api/authors/auth999` },
      ]
    },
    {
      category: '📖 Books',
      ...
      color: 'emerald',
      items: [
        { method: 'GET', path: '/api/books', desc: 'Get all books', example: `curl ${apiUrl}/api/books` },
        { method: 'GET', path: '/api/books?authorId=:id', desc: 'Filter books by author', example: `curl ${apiUrl}/api/books?authorId=auth001` },
        { method: 'GET', path: '/api/books/:id', desc: 'Get book by ID', example: `curl ${apiUrl}/api/books/book001` },
        { method: 'POST', path: '/api/books', desc: 'Create new book', example: `curl -X POST ${apiUrl}/api/books \\
  -H "Content-Type: application/json" \\
  -d '{"id":"book999","title":"New Book","authorId":"auth001","publishedYear":2024,"category":"Technology","isbn":"978-9999999999","pageCount":300,"publisher":"Tech Press","isAvailable":true}'` },
        { method: 'PUT', path: '/api/books/:id', desc: 'Update book', example: `curl -X PUT ${apiUrl}/api/books/book999 \\
  -H "Content-Type: application/json" \\
  -d '{"title":"Updated Title","isAvailable":false}'` },
        { method: 'DELETE', path: '/api/books/:id', desc: 'Delete book', example: `curl -X DELETE ${apiUrl}/api/books/book999` },
      ]
    },
    {
      category: '👥 Members',
      ...
      color: 'purple',
      items: [
        { method: 'GET', path: '/api/members', desc: 'Get all members', example: `curl ${apiUrl}/api/members` },
        { method: 'GET', path: '/api/members/:id', desc: 'Get member by ID', example: `curl ${apiUrl}/api/members/mem001` },
        { method: 'POST', path: '/api/members', desc: 'Create new member', example: `curl -X POST ${apiUrl}/api/members \\
  -H "Content-Type: application/json" \\
  -d '{"id":"mem999","name":"John Doe","email":"john@example.com","phone":"+974-1234-5678","memberType":"student"}'` },
        { method: 'PUT', path: '/api/members/:id', desc: 'Update member', example: `curl -X PUT ${apiUrl}/api/members/mem999 \\
  -H "Content-Type: application/json" \\
  -d '{"name":"John Smith","memberType":"faculty"}'` },
        { method: 'DELETE', path: '/api/members/:id', desc: 'Delete member', example: `curl -X DELETE ${apiUrl}/api/members/mem999` },
      ]
    },
    {
      category: '👔 Staff',
      ...
      color: 'amber',
      items: [
        { method: 'GET', path: '/api/staff', desc: 'Get all staff', example: `curl ${apiUrl}/api/staff` },
        { method: 'GET', path: '/api/staff/:id', desc: 'Get staff by ID', example: `curl ${apiUrl}/api/staff/staff001` },
        { method: 'POST', path: '/api/staff', desc: 'Create new staff', example: `curl -X POST ${apiUrl}/api/staff \\
  -H "Content-Type: application/json" \\
  -d '{"staffId":"staff999","username":"newstaff","password":"pass123","fullName":"New Staff","role":"staff"}'` },
        { method: 'PUT', path: '/api/staff/:id', desc: 'Update staff', example: `curl -X PUT ${apiUrl}/api/staff/staff999 \\
  -H "Content-Type: application/json" \\
  -d '{"fullName":"Updated Name","role":"librarian"}'` },
        { method: 'DELETE', path: '/api/staff/:id', desc: 'Delete staff', example: `curl -X DELETE ${apiUrl}/api/staff/staff999` },
      ]
    },
    {
      ...
      color: 'cyan',
      items: [
        { method: 'GET', path: '/api/transactions', desc: 'Get all transactions', example: `curl ${apiUrl}/api/transactions` },
        { method: 'GET', path: '/api/transactions?memberId=:id', desc: 'Filter by member', example: `curl ${apiUrl}/api/transactions?memberId=mem001` },
        { method: 'GET', path: '/api/transactions?bookId=:id', desc: 'Filter by book', example: `curl ${apiUrl}/api/transactions?bookId=book001` },
        { method: 'GET', path: '/api/transactions/:id', desc: 'Get transaction by ID', example: `curl ${apiUrl}/api/transactions/trans001` },
        { method: 'POST', path: '/api/transactions', desc: 'Create transaction', example: `curl -X POST ${apiUrl}/api/transactions \\
  -H "Content-Type: application/json" \\
  -d '{"id":"trans999","memberId":"mem001","bookId":"book001","borrowDate":"2024-11-17T00:00:00Z","dueDate":"2024-12-01T00:00:00Z","isReturned":false}'` },
        { method: 'PUT', path: '/api/transactions/:id', desc: 'Update transaction', example: `curl -X PUT ${apiUrl}/api/transactions/trans999 \\
  -H "Content-Type: application/json" \\
  -d '{"isReturned":true,"returnDate":"2024-11-20T00:00:00Z"}'` },
        { method: 'DELETE', path: '/api/transactions/:id', desc: 'Delete transaction', example: `curl -X DELETE ${apiUrl}/api/transactions/trans999` },
      ]
    },
  ];
  ...
  const getBorderColor = (method: string) => {
    ...
    return colors[method] || 'border-gray-400';
  };
...
-d '{"username":"admin","password":"admin123"}'`}</pre>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-12 border border-purple-100">
          ...
          <div className="border-l-4 border-purple-500 pl-6 bg-purple-50 rounded-r-xl p-6">
            <div className="flex items-center gap-3 mb-3">
              <span className={`${getMethodColor('POST')} text-sm font-bold px-4 py-1.5 rounded-lg border`}>POST</span>
  ...
  -d '{"username":"admin","password":"admin123"}'`}</pre>
        ...
        {endpoints.map((section, idx) => (
          <div key={idx} className="bg-white rounded-2xl shadow-xl p-8 mb-12 border border-gray-100">
            ...
            <div className="space-y-6">
              {section.items.map((endpoint, endIdx) => (
                <div key={endIdx} className={`border-l-4 ${getBorderColor(endpoint.method)} pl-6 bg-gray-50 rounded-r-xl p-5 hover:bg-gray-100 transition-colors duration-200`}>
                  <div className="flex flex-wrap items-center gap-3 mb-3">
                    <span className={`${getMethodColor(endpoint.method)} text-sm font-bold px-4 py-1.5 rounded-lg border`}>
                      {endpoint.method}
                    </span>
                  ...
                  <p className="text-gray-700 mb-4 font-medium">{endpoint.desc}</p>
                  <div className="bg-gray-900 rounded-xl p-5 overflow-x-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-12 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              { code: '400', name: 'Bad Request', desc: 'Invalid request data', color: 'yellow' },
              { code: '404', name: 'Not Found', desc: 'Resource not found', color: 'red' },
              { code: '409', name: 'Conflict', desc: 'Resource already exists', color: 'orange' },
              { code: '500', name: 'Internal Error', desc: 'Server error', color: 'red' },
            ].map((status, idx) => (
              <div key={idx} className={`p-5 bg-${status.color}-50 border-2 border-${status.color}-200 rounded-xl hover:shadow-lg transition-shadow duration-200`}>
                <div className="flex items-baseline gap-2 mb-2">
                  <code className={`text-${status.color}-700 font-bold text-2xl`}>{status.code}</code>
                  <span className={`text-${status.color}-600 font-semibold`}>{status.name}</span>
                </div>
